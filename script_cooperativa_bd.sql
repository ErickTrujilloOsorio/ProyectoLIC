...
CREATE TABLE IF NOT EXISTS `cooperativa_db`.`creditos` (
  ...
  `nombre_credito` VARCHAR(50) NOT NULL,
  `interes` DECIMAL(8, 2) NOT NULL,
  `descripcion_credito` MEDIUMTEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `cooperativa_db`.`empleados` (
  ...
  CONSTRAINT `fk_estado_id_empleados` FOREIGN KEY (`estado_id`) REFERENCES `cooperativa_db`.`estados` (`idEstado`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_tipo_empleado_id` FOREIGN KEY (`tipo_empleado_id`) REFERENCES `cooperativa_db`.`tipos_empleados` (`idTipo_empleado`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cooperativa_db`.`clientes` (
  ...
  CONSTRAINT `fk_estado_id_clientes` FOREIGN KEY (`estado_id`) REFERENCES `cooperativa_db`.`estados` (`idEstado`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cooperativa_db`.`solicitudes` (
  ...
  CONSTRAINT `fk_cliente_id` FOREIGN KEY (`cliente_id`) REFERENCES `cooperativa_db`.`clientes` (`idCliente`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_empleado_id` FOREIGN KEY (`empleado_id`) REFERENCES `cooperativa_db`.`empleados` (`idEmpleado`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_estado_id_solicitudes` FOREIGN KEY (`estado_id`) REFERENCES `cooperativa_db`.`estados` (`idEstado`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_credito_id` FOREIGN KEY (`credito_id`) REFERENCES `cooperativa_db`.`creditos` (`idCredito`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB;
...
INSERT INTO estados VALUES
  (1, 'activo', 'esta activo'),
  (2, 'inactivo', 'no esta activo'),
  (3, 'Pendiente', 'La solicitud no ha sido asignadada a un empleado'),
  (4, 'En Proceso', 'La solicitud ha sido asignada y se comenzo la gestion'),
  (5, 'Finalizada', 'La gestion ha llegado a una conclusion');
